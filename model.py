...
# From : FrEIA (https://github.com/VLL-HD/FrEIA/)
import FrEIA.framework as Ff
import FrEIA.modules as Fm

__author__ = "Denis Gudovskiy"
...
def load_encoder_arch(c, L):
    ...
    if 'resnet' in c.enc_arch:
        if   c.enc_arch == 'resnet18':
            ...
            if c.backbone_weights != "":
                encoder = wide_resnet50_2(pretrained=False, progress=False) 
                encoder.load_state_dict(torch.load(c.backbone_weights), strict=False)    
            else: 
                encoder = wide_resnet50_2(pretrained=True, progress=False)         
        else:
